<<<<<<<< HEAD:docs/source/policies/load-balancing.md
# Load balancing
...
existing policies, see [migration guide](../migration-guide/migration-guide.md#load-balancing-policies) for more information.
...
When this option is empty (set to `null`), all hosts (unless filtered by other load balancing options)
can be used when connecting to the database.
>>>>>>>> 2f2c36ff (docs: move existing documentation from docs/src to docs/source):docs/source/load-balancing/load-balancing.md
