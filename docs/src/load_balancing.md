...
are still supported by this driver, usage of those policies in new projects is discoursed.
...
`DefaultLoadBalancingPolicy` can be configured only at it's creation.
...
<!-- TODO: Add example import. Smth like: 
const DefaultLoadBalancingPolicy = require("../../main").policies.loadBalancing.DefaultLoadBalancingPolicy;
 -->
...
<!-- Do we want to have this in documentation? 
It's documented in the code, but this options seems like it most user should not 
...
When this options is provided, any of the hosts that is not on the allow
list will be ignored when connecting to the database.
When this option is empty, all hosts (unless filtered by other load balancing options)
