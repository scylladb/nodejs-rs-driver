...
Instead of trying hosts in a fixed order, next host is chosen at random. This is done to avoid situations,
where a trafic from a closed node is redirected to a single next node, potentially overloading it.

If for some reason you relay on this specific order of queueing hosts, you may still replicate old
...
It was replaces with a [new implementation](./load_balancing.md). There are no plans for
re-creating it's functionality.
