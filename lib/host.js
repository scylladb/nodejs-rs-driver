...
class HostMap extends events.EventEmitter {
    ...
    /**
     * Converts a list of Host instances built by Rust into a HostMap.
     * @param {Array.<Host>} hostsList
     * @internal
     * @ignore
     */
    static fromRust(hostsList) {
        ...
        for (const host of hostsList) {
            hostMap.#items.set(host.address, host);
        }
...
// Registers the Host constructor with the native driver, so that Rust can
