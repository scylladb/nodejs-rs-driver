...
function validateFn(fn, name) {
  if (typeof fn !== "function") {
    throw new errors.ArgumentError(`${name || "callback"} is not a function`);
  }
  return fn;
}
...
function stringRepeat(val, times) {
  ...
  if (!times || times < 0) {
    throw new errors.ArgumentError(
      "Cannot repeat string negative amount of times",
    );
  }
  if (times === 1) {
    return val;
...
const VIntCoding = (function () {
  ...
  function readUnsignedVInt(input, offset) {
    ...
    let result = BigInt(sByteInt & firstByteValueMask(size));
