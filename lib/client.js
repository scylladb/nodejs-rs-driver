...
class Client extends events.EventEmitter {
  ...
  async #rustyExecutePrepared(query, params, rustOptions, paged, pageState) {
    ...
    let encoded = encodeParams(prepared.types, unifiedParams, this.#encoder);
  ...
  async #rustyBatch(queries, options) {
    ...
    for (let i = 0; i < queries.length; i++) {
      ...
      let statement = typeof element === "string" ? element : element.query;
      /**
       * @type {Object | Array<any>}
       */
      let params = typeof element !== "string" ? element.params || [] : [];
      /**
       * @type {Array<any>}
       */
      let cleanParams;
      ...
      if (shouldBePrepared) {
        ...
        if (Array.isArray(params)) {
          ...
          cleanParams = utils.adaptNamedParamsPrepared(params, prepared);
        }
      } else {
