 ...
 * @property {String} [localDataCenter] The local data center to use for coordinator selection.
 *
 * When the built-in {@link DefaultLoadBalancingPolicy} does not explicitly configure `preferDatacenter`, this
 * value is used as its preferred data center. Data center failover is disabled unless the policy explicitly
 * configures `permitDcFailover`. An explicitly configured `preferDatacenter` takes precedence. This option affects
 * query plans; it does not restrict cluster discovery or act as a host allow list.
...
function setRustOptions(options) {
    ...
    if (options.policies) {
        if (options.policies.loadBalancing) {
            try {
                const loadBalancingPolicy = options.policies.loadBalancing;
                const loadBalancingConfig =
                    loadBalancingPolicy.getRustConfiguration();

                if (
                    options.localDataCenter !== null &&
                    options.localDataCenter !== undefined &&
                    loadBalancingPolicy instanceof
                        policies.loadBalancing.DefaultLoadBalancingPolicy &&
                    (loadBalancingConfig.preferDatacenter === null ||
                        loadBalancingConfig.preferDatacenter === undefined)
                ) {
                    rustOptions.loadBalancingConfig = {
                        ...loadBalancingConfig,
                        preferDatacenter: options.localDataCenter,
                        permitDcFailover:
                            loadBalancingConfig.permitDcFailover ?? false,
                    };
                } else {
                    rustOptions.loadBalancingConfig = loadBalancingConfig;
                }
            } catch (e) {
