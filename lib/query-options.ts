...
export interface QueryOptions {
    ...
    consistency?: number;
    /**
     * Key-value payload to be passed to the server. On the Cassandra side,
     * implementations of QueryHandler can use this data.
     * [TODO: Add support for this field]
     */
    customPayload?: { [key: string]: any };
    /**
     * Name or instance of the [profile]{@link ExecutionProfile} to
     ...
     */
    hints?: Array<any> | Array<Array<any>>;
    /**
    ...
    keyspace?: string;
    /**
     * Determines if the batch should be written to the batchlog. Only valid for
     * [Client#batch()]{@link Client#batch}, it will be ignored by other methods. Default: true.
     * [TODO: Add support for this field]
     */
    logged?: boolean;
    /**
     * Determines if its a counter batch. Only valid for
    ...
    retry?: RetryPolicy;
    /**
     * Index of the parameters that are part of the partition key to determine
     * the routing.
     * [TODO: Add support for this field]
     */
 ...
 * @ignore
 */
export function queryOptionsIntoWrapper(
    options: ExecutionOptions,
): rust.QueryOptionsWrapper {
    const rustOptions: rust.QueryOptionsObj = Object();

    // `counter`, `keyspace`, `logged` and `prepare` are read straight off the
    // execution options rather than through their getters, so they are always
    // undefined here.
    const asProperties = options as any;
    ...
    rustOptions.counter = asProperties.counter;
    ...
    let timestamp: number | Long | bigint | undefined | null =
    ...
    rustOptions.timestamp = timestamp as bigint | undefined;
    rustOptions.traceQuery = options.isQueryTracing();
    const wrapper = new rust.QueryOptionsWrapper(rustOptions);
    return wrapper;
}
