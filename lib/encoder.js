...
class EncoderMembers {
  ...
  decode(bytes, columnInfo) {
    throw new Error("Using stub of decoder. This is an internal driver error.");
  }
   ...
   * @private
   */
  _decodeCqlLongAsBigInt(bytes) {
    return BigInt.asIntN(
      64,
      (BigInt(bytes.readUInt32BE(0)) << 32n) | BigInt(bytes.readUInt32BE(4)),
    );
  }
   ...
   * @private
   */
  _encodeVarintFromInteger(value) {
    if (typeof value === "number") {
   ...
   * @private
   */
  _encodeRoutingKeyParts(parts, routingIndexes, encodeParam) {
    let totalLength = 0;
   ...
   * @private
   */
  _parseChildTypes(keyspace, typeNames, udtResolver) {
    return Promise.all(
      typeNames.map((name) =>
   ...
   */
  _parseUdtName(typeName, startIndex, length) {
