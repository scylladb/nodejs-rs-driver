...
class Metadata {
     ...
     * @returns {UserDefinedType | null} The UDT definition, or `null` if it does not exist.
     */
    getUdt(keyspaceName, name) {
        let clusterSnapshot = this.#rustClient.getClusterSnapshot();
        return clusterSnapshot.getUdt(keyspaceName, name);
    }
    ...
    getTable(keyspaceName, name) {
        let clusterSnapshot = this.#rustClient.getClusterSnapshot();
        return clusterSnapshot.getTable(keyspaceName, name);
    }
    ...
    getMaterializedView(keyspaceName, name) {
        let clusterSnapshot = this.#rustClient.getClusterSnapshot();
        return clusterSnapshot.getMaterializedView(keyspaceName, name);
    }
